...
public class HBaseTableScanner {
    ...
    private static long[] getQuarterTimeRange(String tableName) {
        ...
        // 解析年份和季度部分
        String yearQuarterPart = parts[1];
        int qIndex = yearQuarterPart.indexOf('Q');
        if (qIndex < 0) {
            throw new IllegalArgumentException("Invalid table name format: " + tableName);
        }

        int year = Integer.parseInt(yearQuarterPart.substring(0, qIndex));
        int quarter = Integer.parseInt(yearQuarterPart.substring(qIndex + 1));
