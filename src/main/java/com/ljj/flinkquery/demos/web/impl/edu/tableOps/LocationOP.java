...
public class LocationOP {
    ...
    public static Location GETLonNearest(double Longitude, List<Location> roadDataList) throws IOException {
        int i = 0;
        int j = 0;
        double diff=10000;
        // 取出纬度差值最小的十条数据
        for (Location l : roadDataList) {
            double temp = Math.abs(l.getLongitude() - Longitude);
            if (temp < diff) {
                diff = temp;
                j=i;
            }
        ...
        return roadDataList.get(j);
    }

        public static Location GETLatNearest(double Latitude, List<Location> roadDataList) throws IOException {
        int i = 0;
        int j = 0;
        double diff=10000;
        // 取出纬度差值最小的十条数据
        for (Location l : roadDataList) {
            double temp = Math.abs(l.getLatitude() - Latitude);
            if (temp < diff) {
                diff = temp;
                j=i;
            }
